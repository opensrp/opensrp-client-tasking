...
public class TaskRegisterFragmentInteractor extends BaseInteractor implements TaskRegisterFragmentContract.Interactor {
    ...
    public TaskRegisterFragmentInteractor(TaskRegisterFragmentContract.Presenter presenter,
        ...
        locationRepository = CoreLibrary.getInstance().context().getLocationRepository(); //RevealApplication.getInstance().getLocationRepository();
    ...
    private String nonRegisteredStructureTasksSelect(String mainCondition) {
        ...
        return TaskingLibrary.getInstance().getTaskingLibraryConfiguration().nonRegisteredStructureTasksSelect(mainCondition);
    }
    ...
    private String groupedRegisteredStructureTasksSelect(String mainCondition) {
        ...
        String[] columns = ArrayUtils.add(mainColumns(tableName), String.format("%s.%s||' '||%s.%s as %s ", "", "", "", LAST_NAME, ""));//FAMILY_MEMBER, FIRST_NAME, FAMILY_MEMBER, LAST_NAME, FAMILY_MEMBER_NAMES));
        ...
        return String.format(" SELECT %s.* , SUM(CASE WHEN status='%s' THEN 1 ELSE 0 END ) AS %s , COUNT(_id ) AS %s, " +
                ...
                GROUPED_TASKS, COMPLETED.toString(), COMPLETED_TASK_COUNT, TASK_COUNT, CODE, BUSINESS_STATUS, GROUPED_STRUCTURE_TASK_CODE_AND_STATUS, "", "") + structureTasksQueryBuilder +//FAMILY_MEMBER_NAMES, FAMILY_MEMBER_NAMES) + structureTasksQueryBuilder +
                String.format(" ) AS %s GROUP BY %s ", GROUPED_TASKS, STRUCTURE_ID);*/
        return TaskingLibrary.getInstance().getTaskingLibraryConfiguration().groupedRegisteredStructureTasksSelect(mainCondition);
    }
    ...
    private String[] mainColumns(String tableName) {
        ...
        return TaskingLibrary.getInstance().getTaskingLibraryConfiguration().taskRegisterMainColumns(tableName);
    }
